...
class Game:
    ...
    def _start_new_round(self) -> None:
        self.target = self._create_random_target()
        self.move_count = 0
        ...
        for i, p in enumerate(self.players):
            if p.bid is not None:
                if best_bid is None or p.bid < best_bid:
                    best_bid = p.bid
                    best_index = i
        self.active_player_index = best_index
        self.state = "solving"
        self.move_count = 0
    ...
    def _handle_victory(self) -> None:
        if self.active_player_index is not None:
            ...
            if player.bid is None or self.move_count <= player.bid:
                player.score += 1
        self.round_index += 1
    ...
    def update(self, dt: float) -> None:
        if self.state == "bidding":
            ...
            if self.sand_timer <= 0:
                self._end_bidding()
        elif self.state == "solving" and self.target is not None:
            target_robot = self.robots[self.target.robot_index]
            if (target_robot.col, target_robot.row) == (self.target.goal_col, self.target.goal_row):
    ...
    def _draw_goal_cell(self, board_width: int, board_height: int) -> None:
        if self.target is None:
            return
        rect = cell_rect(self.target.goal_col, self.target.goal_row, board_width, board_height)
        pygame.draw.rect(self.screen, GOAL_OUTLINE_COLOR, rect, 3)

    def _draw_goal_symbol(self, board_width: int, board_height: int) -> None:
        if self.target is None:
            return
        rect = cell_rect(self.target.goal_col, self.target.goal_row, board_width, board_height)
        cx = rect.centerx
    ...
    def _draw_side_panel(self, width: int, height: int, board_width: int) -> None:
        ...
        if self.target is not None:
            ...
            robot_color = self.robots[self.target.robot_index].color
            circle_x = target_rect.x + 28
            circle_y = target_rect.centery
            pygame.draw.circle(self.screen, robot_color, (circle_x, circle_y), 12)
            pygame.draw.circle(self.screen, MENU_BUTTON_COLOR, (circle_x, circle_y), 12, 2)

            label_surf = self.font.render("Robot objectif", True, HUD_TEXT_COLOR)
            label_rect = label_surf.get_rect(topleft=(circle_x + 24, target_rect.y + 10))
            self.screen.blit(label_surf, label_rect)

            info_surf = self.font.render("Atteindre la case cible sur le plateau", True, HUD_TEXT_COLOR)
        ...
        if self.state == "bidding":
            ...
            seconds = max(0, int(self.sand_timer))
            timer_label = self.font.render("Temps restant", True, HUD_TEXT_COLOR)
    ...
    def draw(self) -> None:
        ...
        self._draw_goal_cell(board_width, board_height)
        self._draw_goal_symbol(board_width, board_height)
